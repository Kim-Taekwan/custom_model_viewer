...
if __name__ == '__main__':
    ...
    floor = Cube(Vec3(28, 0.5, 27.5), Vec4(245, 245, 245, 255))
    wall_r = Cube(Vec3(0.5, 27, 27.5), Vec4(200, 50, 50, 255))
    wall_g = Cube(Vec3(0.5, 27, 27.5), Vec4(50, 150, 70, 255))
    wall_w = Cube(Vec3(27, 27, 0.5), Vec4(245, 245, 245, 255))

    cube = Cube(Vec3(10, 10, 8), Vec4(220, 220, 220, 225))
    glass = Sphere(30, 30, 4, Vec4(180,240,250, 50))
    metal = Sphere(30, 30, 2.5, Vec4(130, 130, 130, 255))
    
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=0, y=0, z=0)), floor)
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=-13.75, y=13.75, z=0)), wall_r)
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=13.75, y=13.75, z=0)), wall_g)
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=0, y=13.75, z=-13.5)), wall_w)
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=0, y=27.5, z=0)), floor)

    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=-4.5, y=5.25, z=-6.5)) @ Mat4.from_rotation(0.7, vector=Vec3(0, 1, 0)), cube)
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=6, y=4.25, z=3.5)), glass)
    renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=-6, y=2.75, z=6.5)), metal)

    renderer.add_area_light(width=6, depth=5, interval=1, transform=Mat4.from_translation(Vec3(0, 26.5, 0)), intensity=0.05)
    #area_light = Cube(Vec3(6, 0.1, 5))
    #renderer.add_primitive(Mat4.from_translation(vector=Vec3(x=0, y=27.25, z=0)), area_light)
